...
#define FILENAME "SingleChatThreadSetting.config"
...
int main()
{
	...
	while (1)
	{
		...
		wprintf(L"LogicQ  :  %d \n", ChatInstance->GetLogicQueueSize());
		wprintf(L"Contents \n");
		...
		wprintf(L"DCUnloginTimeout  :  %d     DCLoginTimeout  :  %d \n", ChatInstance->GetDCUnloginTimeout(), ChatInstance->GetDCLoginTimeout());
		...
		wprintf(L"RecvTPS  :  %d     SendTPS  :  %d \n", ChatInstance->GetRecvMessageTPS(), ChatInstance->GetSendMessageTPS());
